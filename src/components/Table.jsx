...
const columns = [
  ...
  { id: "nombre_completo", label: "Nombre y Apellido", minWidth: 100, align: "center" },
  {
    id: "hora_entrada",
...
export function TableWorkers({data, title, date}) {
  ...
  return (
    <Box sx={{ display:"flex", flexDirection:"column", justifyContent:"flex-start", alignItems:"center", height:"100vh"}}>
          ...
          <TableBody>
            {filteredUsers?.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => (
              <React.Fragment key={row.cedula}>
                <TableRow hover tabIndex={-1} style={{ backgroundColor: index % 2 === 0 ? 'white' : '#f5f5f5'}}>
                  {columns.map((column) => {
                    ...
                    >
                      ...
                      <Box padding={3}
                        ...
                        <Box sx={{ display: "flex", justifyContent: "center", width: "100%", flexWrap: "wrap", gap: "10px" }} >
                          <div>
                            <b>Telefono:</b> {row.telefono === "" ? "Sin datos" : row.telefono}
                          </div>
                          <div>
                            <b>Correo:</b> {row.correo === "" ? "Sin datos" : row.correo}
                          </div>
                          <div>
                            <b>Genero:</b> {row.genero === "" ? "Sin datos" : row.genero}
                          </div>
                          <div>
                            <b>Dirreción:</b> {row.direccion === undefined ? "Sin dirección " : row.direccion}
                          </div><div>
                            <b>Estatus:</b> {row.estatus ? "Activo " : "Inactivo"}
                          </div>
                        </Box>
                      </Box>
                    </Collapse>
